...
<div x-data="{ show: false, name: '{{ $name }}' }" x-on:open-modal.window="show = ($event.detail.name === name)"
    x-on:close-modal.window="show = false" x-on:keydown.escape.window="$dispatch('close-modal')" x-show="show" x-transition.opacity
    class="fixed inset-0 z-50 flex items-center justify-center" style="display: none">
    {{-- Faded Background --}}
    <div class="absolute inset-0 bg-black opacity-40 backdrop-blur-sm" x-on:click="$dispatch('close-modal')"></div>

    {{-- Modal Box --}}
    <div x-transition class="relative bg-white rounded-2xl shadow-lg w-full max-w-lg mx-4 p-6 z-10">
        @php
